...
export default function WebGLStarter() {
	...
	const [GameState, setGameState] = React.useState<"BET" | "PLAYING" | "GAMEEND">("BET");
	...
	useEffect(() => {
	  ...
	  const connectSocket = () => {
    const socket = new SockJS("http://localhost:8585/ws-game");
  ...
  const handleMultiplierMessage = (round) => {
  const { state, currentMultiplier } = round;
	...
	return (
		<div className="crash-container">
            ...
            <div className="waiting">
              <div
                style={{
                  width: `${Math.max(0, (5000 - waiting) * 100 / 5000)}%`,
              ...
              ></div>
					...
					<div className={`crashtext ${GameState === "GAMEEND" && "red"}`}>
						...
						<div>
							 {multiplier.toFixed(2)} <span className="font-[900]">x</span>
